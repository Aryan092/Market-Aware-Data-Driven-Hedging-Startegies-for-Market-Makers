...
def SPXDataPlot(datapath, surfacespath):
    ...
    data['date_dt_1'] = pd.to_datetime(data['date'],format='%Y-%m-%d')
    data['days'] = data['date_dt_1'].dt.day
    ...
    data_temp = data[data['date_dt']==dates[0]]
    ...
    SPY = yf.download(y_symbols, start=startdate, end=enddate, auto_adjust=False)
    prices = np.array(SPY['Adj Close']['SPY']*10)
    ...
    SPY2 = yf.download(y_symbols, start=startdate, end=enddate, auto_adjust=False)
    prices_prev = np.zeros(len(prices))
    prices_prev[1:] = prices[0:-1]
    prices_prev[0] = 10 * SPY2['Adj Close'].iloc[0].item()
...
def DataPreprocesssing(datapath, surfacepath):
    ...
    log_iv_t = np.log(surfaces_transform[22:]) #looooool
...
def VolGAN(datapath,surfacepath, tr, noise_dim = 16, hidden_dim = 8, n_epochs = 1000,n_grad = 100, lrg = 0.0001, lrd = 0.0001, batch_size = 100, device = 'cpu'):
    ...
    print("=" * 50)
    print(f"Training samples: {true_train.shape[0]}")
    print(f"Test samples: {true_test.shape[0]}")
    print(f"Condition dimensions: {condition_train.shape[1]}")
    print(f"True output dimensions: {true_train.shape[1]}")
    print()
    
    print("TRUE_TRAIN statistics:")
    print(f"  Min: {true_train.min().item():.6f}")
    print(f"  Max: {true_train.max().item():.6f}")
    print(f"  Mean: {true_train.mean().item():.6f}")
    print(f"  Std: {true_train.std().item():.6f}")
    print(f"  Contains NaN: {torch.isnan(true_train).any().item()}")
    print(f"  Contains Inf: {torch.isinf(true_train).any().item()}")
    print()
    
    print("CONDITION_TRAIN statistics:")
    print(f"  Min: {condition_train.min().item():.6f}")
    print(f"  Max: {condition_train.max().item():.6f}")
    print(f"  Mean: {condition_train.mean().item():.6f}")
    print(f"  Std: {condition_train.std().item():.6f}")
    print(f"  Contains NaN: {torch.isnan(condition_train).any().item()}")
    print(f"  Contains Inf: {torch.isinf(condition_train).any().item()}")
    print()
    
    print("m_in (normalization for input):")
    print(f"  {m_in}")
    print(f"  Contains NaN: {torch.isnan(m_in).any().item()}")
    print(f"  Contains Inf: {torch.isinf(m_in).any().item()}")
    print()
    
    print("sigma_in (normalization for input):")
    print(f"  {sigma_in}")
    print(f"  Contains NaN: {torch.isnan(sigma_in).any().item()}")
    print(f"  Contains Inf: {torch.isinf(sigma_in).any().item()}")
    print("=" * 50)
    print()
    # ===== END DATA DIAGNOSTIC =====
    ...
    test_noise = torch.randn((10, noise_dim), device=device)
    test_cond = condition_train[:10]
    test_fake = gen(test_noise, test_cond)
    test_real = true_train[:10]
    test_fake_and_cond = torch.cat((test_cond, test_fake), dim=-1)
    test_real_and_cond = torch.cat((test_cond, test_real), dim=-1)

    disc_fake_out = disc(test_fake_and_cond)
    disc_real_out = disc(test_real_and_cond)

    print("Disc fake output min/max:", disc_fake_out.min().item(), disc_fake_out.max().item())
    print("Disc real output min/max:", disc_real_out.min().item(), disc_real_out.max().item())
    print("Contains NaN (fake):", torch.isnan(disc_fake_out).any().item())
    print("Contains Inf (fake):", torch.isinf(disc_fake_out).any().item())
    print("Contains NaN (real):", torch.isnan(disc_real_out).any().item())
    print("Contains Inf (real):", torch.isinf(disc_real_out).any().item())
